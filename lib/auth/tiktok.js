...
async function requestToken(code, codeVerifier, includeClientSecret) {
  const cfg = getTikTokConfig();
  const payload = {
    client_key: cfg.clientKey,
...
export async function exchangeCodeForToken(code, codeVerifier) {
  const primary = await requestToken(code, codeVerifier, true);
  let res = primary;

  if (!primary.ok) {
    const fallback = await requestToken(code, codeVerifier, false);
    if (fallback.ok) {
      res = fallback;
    }
  }
