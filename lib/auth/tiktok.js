...
function isSandboxEnabled() {
  const raw = String(process.env.TIKTOK_USE_SANDBOX || process.env.TIKTOK_SANDBOX || "").toLowerCase();
...
function getApiBaseCandidates() {
  ...
  const secondary = primary === SANDBOX_API_BASE ? PROD_API_BASE : SANDBOX_API_BASE;

  // In production, always hit the stable public host first to avoid sandbox DNS issues.
  if (process.env.NODE_ENV === "production") {
    const allowSandboxFallback = isSandboxEnabled();
    return allowSandboxFallback ? [PROD_API_BASE, SANDBOX_API_BASE] : [PROD_API_BASE];
  }
...
export async function exchangeCodeForToken(code, codeVerifier) {
  ...
  for (const tokenUrl of tokenUrls) {
    try {
      ...
      if (withSecret.ok) {
        const payload = await withSecret.json().catch(() => ({}));
        if (payload?.access_token || payload?.data?.access_token) {
          return payload;
        }
        lastErrorText = JSON.stringify(payload);
      }

      const withPkceOnly = await requestToken(code, codeVerifier, false, tokenUrl);
      if (withPkceOnly.ok) {
        const payload = await withPkceOnly.json().catch(() => ({}));
        if (payload?.access_token || payload?.data?.access_token) {
          return payload;
        }
        lastErrorText = JSON.stringify(payload);
      ...
      const errorText = await withPkceOnly.text().catch(() => "");
      if (errorText) {
        lastErrorText = errorText;
      }
    } catch (error) {
      lastErrorText = String(error?.message || error || "fetch failed");
      continue;
    }
  }
